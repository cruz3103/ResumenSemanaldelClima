...
# =====================
T_base = 10
df['HDD_15min'] = np.where(df['Temperature'] < T_base, (T_base - df['Temperature']) * 0.25, 0)
df = df.sort_values('Fecha')

# HDD diario
df['Dia'] = pd.to_datetime(df['Fecha'].dt.date)
hdd_diaria = df.groupby('Dia')['HDD_15min'].sum().reset_index()
hdd_diaria = hdd_diaria.sort_values('Dia')
hdd_diaria['HDD_60d_acumulado'] = hdd_diaria['HDD_15min'].rolling(window=60, min_periods=1).sum()
df = df.merge(hdd_diaria[['Dia', 'HDD_60d_acumulado']], on='Dia', how='left')

# =====================
# Precipitación y Radiación (60d)
# =====================
...
# HDD acumulado últimos 60 días desde cálculo diario
hdd_diaria['Semana'] = pd.to_datetime(hdd_diaria['Dia']).dt.to_period('W').apply(lambda r: r.start_time)
hdd_semanal_60d = hdd_diaria.groupby('Semana')['HDD_60d_acumulado'].last().reset_index()
hdd_semanal_60d.columns = ['Semana', 'HDD acumulado últimos 60 días']
resumen = resumen.merge(hdd_semanal_60d, on='Semana', how='left')
...
descripcion = pd.DataFrame({
    ...
    'Descripción': [
        ...
        'Total semanal estimado de precipitación (mm)',
        ...
        'Total semanal de HDD: suma de (T - T_base) * 0.25 cada 15 min',
        'Acumulado de HDD en los 60 días anteriores',
        'Cantidad de días con heladas (T < 4 °C)',
        'Cantidad de días con radiación solar > 0 W/m²'
    ]
})
...
plt.title('Días con heladas por semana (T < 4 °C)')
plt.xlabel('Semana')
...
plt.title('HDD acumulados (últimos 60 días)')
plt.xlabel('Semana')
plt.ylabel('HDD')
plt.grid(True)
plt.xticks(rotation=45)
